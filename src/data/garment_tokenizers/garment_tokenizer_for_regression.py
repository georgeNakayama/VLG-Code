...
from data.garment_tokenizers.utils import (
...
class GarmentTokenizerForRegression(GarmentTokenizer):
    ...
    def get_zero_vertices(self):
        return -torch.tensor(self.gt_stats.vertices.shift) / torch.tensor(
    ...
    def _pattern_as_list_sf(
        self, pattern: SF_SewingPattern, as_quat=False, endpoint_first=True
    ):
        panel_order = pattern.panel_order(filter_nones=True)
        panel_edges, panel_rotations, panel_translations = [], [], []
        panel_names = []
        for panel_name in panel_order:
            panel_edge, rotation, translation = pattern.panel_as_list(
                panel_name,
                rot_in_euler=not as_quat,
                version=3,
            ...
            p_name = (
                pattern.panel_classifier.class_name(
                    pattern.panel_classifier.class_idx(
                        pattern.template_name, panel_name
                    )
                )
                if pattern.panel_classifier is not None
                else panel_name
            )
    ...
    def encode(
        self, pattern: Union[GCD_NNSewingPattern, SF_SewingPattern], return_type="pt"
    ):
        assert (
            self.panel_edge_type_indices is not None
        ), "Panel edge type indices not set"
        ...
        pattern_edges, panel_names, panel_rotations, panel_translations, stitches = (
            self._pattern_as_list_sf(pattern, as_quat=True, endpoint_first=True)
        ...
        for panel_edges, panel_name, panel_tran, panel_rot in zip(
            ...
            transl_params = (
                panel_tran.flatten() - self.gt_stats.translations.shift
            ) / self.gt_stats.translations.scale
            rot_params = (
                panel_rot.flatten() - self.gt_stats.rotations.shift
            ) / self.gt_stats.rotations.scale
            trans_params = np.concatenate(
                [transl_params.flatten(), rot_params.flatten()]
            )
            transformations.append(trans_params)
            params_output[
                self.panel_edge_type_indices.get_token_indices(PanelEdgeTypeV3.MOVE)
            ].append(trans_params)
            out_description += [PanelEdgeTypeV3.MOVE.value]
            for edge_id, panel_edge in enumerate(panel_edges):
                edge_type: PanelEdgeTypeV3 = panel_edge[0]
                edge_params = (
                    panel_edge[1].reshape(-1, 2) - self.gt_stats.vertices.shift
                ) / self.gt_stats.vertices.scale
                if edge_type.is_closure():
                    endpoints.append(
                        -np.array(self.gt_stats.vertices.shift)
                        / np.array(self.gt_stats.vertices.scale)
                    )
                else:
                ...
                if param_num > 0:
                    token_indices = self.panel_edge_type_indices.get_token_indices(
                        edge_type
                    )
        ...
        if return_type == "pt":
            params_output = {
                k: torch.from_numpy(v).float() for k, v in params_output.items()
            }
    ...
    def decode(self, output_dict: dict[str, Any], tokenizer: PreTrainedTokenizer):
        ...
        garment_ends = np.where(
            output_ids
            == self.special_token_indices.get_token_indices(SpecialTokensV2.PATTERN_END)
        )[0]
        ...
        for k, v in output_param_dict.items():
            ...
            if v.shape[0] != param_mask.sum():
                log.error(
                    f"Param {k} has wrong shape {v.shape} vs {param_mask.sum()}. Output is {text_output}"
                )
                output_param_dict[k] = v[: param_mask.sum()]

        pattern_dict, error_type = self.decode_pattern(
            pattern_tokens, output_param_dict, tokenizer
        )
    ...
    def decode_tensor(self, output_tensor, param_dict, tokenizer: PreTrainedTokenizer):
        """Decode output ids to text. Note that this does not work for editing samples as we don't have an input mask. cf. `decode`"""
    ...
    def decode_pattern(
        ...
        if (
            ...
            return {
                "panels": {},
                "stitches": [],
            }, DecodeErrorTypes.UNMATCHED_PANEL_TOKENS
        ...
        for i in range(n_panels):
            ...
            for j in range(len(commands)):
                ...
                if param_array is not None:
                    assert (
                        len(param_array) > 0
                    ), f"The param array is too short for command: {command}"
                ...
                if edge_type == PanelEdgeTypeV3.CLOSURE_LINE:
                    ...
                    endpoint = params * np.array(
                        self.gt_stats.vertices.scale
                    ) + np.array(self.gt_stats.vertices.shift)
                elif edge_type == PanelEdgeTypeV3.CURVE:
                    params = params.reshape(2, 2) * np.array(
                        self.gt_stats.vertices.scale
                    ) + np.array(self.gt_stats.vertices.shift)
                    ...
                    params = params * np.array(self.gt_stats.vertices.scale) + np.array(
                        self.gt_stats.vertices.shift
                    )
                    ctrl_pt = control_to_relative_coord(
                        last_point, np.array([0, 0]), params
                    )
                    edge_dict["curvature"] = {"type": "quadratic", "params": [ctrl_pt]}
                elif edge_type == PanelEdgeTypeV3.CUBIC:
                    params = params.reshape(3, 2) * np.array(
                        self.gt_stats.vertices.scale
                    ) + np.array(self.gt_stats.vertices.shift)
                    endpoint, abs_ctrl_pt1, abs_ctrl_pt2 = (
                        params[0],
                        params[1],
                        params[2],
                    )
                    ...
                    ctrl_pt1 = control_to_relative_coord(
                        last_point, np.array([0, 0]), params[0]
                    )
                    ctrl_pt2 = control_to_relative_coord(
                        last_point, np.array([0, 0]), params[1]
                    )
                    edge_dict["curvature"] = {
                        "type": "cubic",
                        "params": [ctrl_pt1, ctrl_pt2],
