...
from .utils import (
    arc_rad_flags_to_three_point,
    control_to_abs_coord,
    discretize,
    control_to_relative_coord,
...
from .special_tokens import (
    SpecialTokensV2,
    SpecialTokensIndices,
    PanelEdgeTypeV3,
    PanelEdgeTypeIndices,
    DecodeErrorTypes,
)
...
class GarmentTokenizer:
    def __init__(
        ...
        if self.include_template_name:
            assert (
                self.sf_only
            ), "include_template_name can only be used with sewfactory dataset"

        self.panel_edge_type_indices: PanelEdgeTypeIndices | None = None
        self.special_token_indices: SpecialTokensIndices | None = None
        self.bin_name2bin_number = {
            k: i for i, k in enumerate(self.get_bin_token_names())
        }
    ...
    def get_stitch_tag_names(self):
        return [f"<stitch_tag_{i}>" for i in range(self.num_tags)] + [
            "<stitch_tag_null>"
        ]
    ...
    def set_token_indices(self, token2idx: dict):
        self.special_token_indices = SpecialTokensIndices(token2idx)
        self.panel_edge_type_indices = PanelEdgeTypeIndices(
            token2idx, rot_as_quat=False
        )
    ...
    def encode_list(
        self, pattern_list: list[GCD_NNSewingPattern | SF_SewingPattern]
    ):
        encoded_list = []
        for pattern in pattern_list:
            encoded_list.append(self.encode(pattern))
        return encoded_list

    def encode(self, pattern: GCD_NNSewingPattern | SF_SewingPattern):
        if self.sf_only:
            assert isinstance(
                pattern, SF_SewingPattern
            ), "Pattern must be an instance of SF_SewingPattern"

        bin_tokens = self.get_bin_token_names()
        if self.encode_stitches_as_tags:
            tag_tokens = self.get_stitch_tag_names()

        pattern_edges, panel_names, panel_rotations, panel_translations, stitches = (
            self._pattern_as_list_sf(pattern)
            if isinstance(pattern, SF_SewingPattern)
            else self._pattern_as_list_gcd(pattern)
        )
        stitches = (
        ...
        for panel_edges, panel_name, panel_tran, panel_rot in zip(
            pattern_edges, panel_names, panel_translations, panel_rotations
        ):
            out_description += [SpecialTokensV2.PANEL_START.value, panel_name]
            trans_params = discretize(
                panel_tran.reshape(-1, 3),
                self.bin_size,
                self.gt_stats.translations.shift,
                self.gt_stats.translations.scale,
            )
            rot_params = discretize(
                panel_rot.reshape(-1, 3),
                self.bin_size,
                self.gt_stats.rotations.shift,
                self.gt_stats.rotations.scale,
            )
            out_description += (
                [PanelEdgeTypeV3.MOVE.value]
                + [bin_tokens[p] for p in trans_params.flatten().tolist()]
                + [bin_tokens[p] for p in rot_params.flatten().tolist()]
            )
            for edge_id, panel_edge in enumerate(panel_edges):
                edge_type = panel_edge[0]
                edge_params = discretize(
                    panel_edge[1].reshape(-1, 2),
                    self.bin_size,
                    self.gt_stats.vertices.shift,
                    self.gt_stats.vertices.scale,
                )
                ...
                if param_num > 0:
                    out_description += [
                        bin_tokens[p]
                        for p in edge_params.flatten()[:param_num].tolist()
                    ]
                if self.encode_stitches_as_tags:
                    # last entry is null
    ...
    def _pattern_as_list_gcd(
        self, pattern: GCD_NNSewingPattern, as_quat=False, endpoint_first=False
    ):
        ...
        for panel_name in panel_order:
            ...
            for i, edge in enumerate(panel["edges"]):
                ...
                if "curvature" in edge:
                    ...
                    if edge["curvature"]["type"] == "circle":
                        ...
                        _, _, coords = arc_rad_flags_to_three_point(
                            endpoints[0],
                            endpoints[1],
                            params[0],
                            params[1],
                        ...
                        params = (
                            np.concatenate([coords, endpoints[1]])
                            if not endpoint_first
                            else np.concatenate([endpoints[1], coords])
                        )
                    ...
                    # in case of a curve
                        ...
                        if self.convert_qradratic_to_cubic:
                            ...
                            cubic_1 = start + 2.0 / 3.0 * (cp - start)
                            cubic_2 = end + 2.0 / 3.0 * (cp - end)
                            edge_type = PanelEdgeTypeV3.CUBIC
                            params = (
                                np.concatenate(
                                    [
                                        control_to_abs_coord(
                                            endpoints[0], endpoints[1], cubic_1
                                        ),
                                        control_to_abs_coord(
                                            endpoints[0], endpoints[1], cubic_2
                                        ),
                                        endpoints[1],
                                    ]
                                )
                ...
                if i == len(panel["edges"]) - 1:
                    ...
                    # params = params[:-2]
                    params = (
                        params[:-2] if not endpoint_first else params[2:]
            ...
            translation, _ = panel_universal_transtation(
                vertices, panel["rotation"], panel["translation"]
            )
            panel_translations.append(translation)
            panel_rotations.append(
                np.array(panel["rotation"])
                if not as_quat
                else Rotation.from_euler(
    ...
    def _to_verts(self, panel_edges):
        ...
        for edge in panel_edges:
            ...
            next_curvature = (
                vert_list[-1] + edge[2] * edge[:2]
            )  # X curvature coordinate
            next_curvature = (
                next_curvature + edge[3] * edge_perp
            )  # Y curvature coordinate
    ...
    def evaluate_patterns(
        self,
        pred_patterns: list[GCD_NNSewingPattern],
        gt_patterns: list[GCD_NNSewingPattern],
    ):
        ...
        for k, (pred_pattern, gt_pattern) in enumerate(zip(pred_patterns, gt_patterns)):
            ...
            for i in range(len(pred_panel_names)):
                try:
                    edges, rot, transl, aug_edges = pred_pattern.panel_as_numeric(
                        pred_panel_names[i], pad_to_len=None
                    )
                except EmptyPanelError:
                    continue
                if pred_panel_names[i] in gt_panel_names:
                    gt_edges, gt_rot, gt_transl, gt_aug_edges = (
                        gt_pattern.panel_as_numeric(
                            pred_panel_names[i], pad_to_len=None
                        )
                    )
                else:
            ...
            if not correct:
                continue
            garment_residual_error, garment_translation_l2, garment_rotation_l2 = (
                0,
                0,
                0,
            )
            ...
            for i in range(len(pred_panel_names)):
                ...
                gt_edges, gt_rot, gt_transl, gt_aug_edges = gt_pattern.panel_as_numeric(
                    gt_panel_names[i], pad_to_len=None
                )
                ...
                if len(gt_edges) < max_length:
                    gt_edges = np.pad(
                        gt_edges,
                        ((0, max_length - len(gt_edges)), (0, 0)),
                        mode="constant",
                        constant_values=0,
                    )
                residual_error = np.linalg.norm(
                    self._to_verts(edges) - self._to_verts(gt_edges), axis=1, ord=2
                ).mean()
                transl_l2 = np.linalg.norm(transl - gt_transl, ord=2)
                rot_l2 = min(
                    np.linalg.norm(rot - gt_rot, ord=2),
                    np.linalg.norm(rot + gt_rot, ord=2),
                )
            ...
            total_num_edge_in_correct_acc[k] = num_edge_in_correct / max(
                len(pred_panel_names), 1
            )
            total_residual_error[k] = garment_residual_error / max(
                len(pred_panel_names), 1
            )
            total_translation_l2[k] = garment_translation_l2 / max(
                len(pred_panel_names), 1
            )
            ...
            if self.encode_stitches_as_tags:
                ...
                for stitch_list in pred_stitches:
                    pred_stitches_dict[
                        (stitch_list[0]["panel"], stitch_list[0]["edge"])
                    ] = (stitch_list[1]["panel"], stitch_list[1]["edge"])
                    pred_stitches_dict[
                        (stitch_list[1]["panel"], stitch_list[1]["edge"])
                    ] = (stitch_list[0]["panel"], stitch_list[0]["edge"])
                for stitch_list in gt_stitches:
                    n_correct += (
                        1
                        if (stitch_list[0]["panel"], stitch_list[0]["edge"])
                        in pred_stitches_dict
                        and pred_stitches_dict[
                            (stitch_list[0]["panel"], stitch_list[0]["edge"])
                        ]
                        == (stitch_list[1]["panel"], stitch_list[1]["edge"])
                        else 0
        ...
        sorted_inds = np.arange(len(pred_patterns))[
            np.argsort(total_residual_error.cpu().numpy())[::-1]
        ]
    ...
    def _pattern_as_list_sf(
        self, pattern: SF_SewingPattern, as_quat=False, endpoint_first=False
    ):
        panel_order = pattern.panel_order(filter_nones=True)
        all_panel_edges, panel_rotations, panel_translations = [], [], []
        for panel_name in panel_order:
                ...
                if "curvature" in edge:
                    ...
                    params = (
                        np.concatenate(
                            [
                                control_to_abs_coord(
                                    endpoints[0], endpoints[1], edge["curvature"]
                                ),
                                endpoints[1],
                            ]
                        )
    ...
    def decode(self, output_dict: dict, tokenizer: PreTrainedTokenizer):
        """Decode output ids to text"""
        output_ids = output_dict["output_ids"]
        input_mask = output_dict["input_mask"]
        text_output = tokenizer.decode(
            output_ids[output_ids != IMAGE_TOKEN_INDEX], skip_special_tokens=True
        )
        output_ids = output_ids.cpu().numpy().copy()
        garment_ends = np.where(
            np.logical_and(
                output_ids
                == self.special_token_indices.get_token_indices(
                    SpecialTokensV2.PATTERN_END
                ),
                input_mask,
            )
        )[0]
        ...
        if (
            len(garment_starts) != len(garment_ends)
            or len(garment_starts) == 0
            or len(garment_ends) == 0
            or np.any(garment_starts > garment_ends)
        ):
            log.error(
                "Garment Decoding Error: Unmatched or Invalid number of garment starts and ends"
            )
    ...
    def decode_pattern(
        self, token_sequence: np.ndarray, tokenizer: PreTrainedTokenizer
    ):
        assert (
            self.bin_idx2bin_number is not None
            and self.bin_idx2bin_name is not None
            and self.bin_name2bin_idx is not None
            and self.special_token_indices is not None
            and self.panel_edge_type_indices is not None
        ), "token indices needs to be set before decoding"
        # find the panel starts and ends
        panel_starts = np.where(
            token_sequence
            == self.special_token_indices.get_token_indices(SpecialTokensV2.PANEL_START)
        )[0]
        panel_ends = np.where(
            token_sequence
            == self.special_token_indices.get_token_indices(SpecialTokensV2.PANEL_END)
        )[0]
        if (
            len(panel_starts) != len(panel_ends)
            or len(panel_starts) == 0
            or len(panel_ends) == 0
            or np.any(panel_starts > panel_ends)
        ):
            log.error(
                "Garment Decoding Error: Unmatched or Invalid number of panel starts and ends"
            )
        ...
        for i in range(n_panels):
            ...
            commands = np.isin(
                panel, self.panel_edge_type_indices.get_all_indices()
            ).nonzero()[0]
            if len(commands) == 0:
            ...
            for j in range(len(commands)):
                ...
                edge_type = self.panel_edge_type_indices.get_index_token(
                    panel[command].item()
                )
                edge_params = panel[command + 1 : command_end]

                num_should_predict = edge_type.get_num_params() + int(
                    self.encode_stitches_as_tags and edge_type != PanelEdgeTypeV3.MOVE
                )
                if len(edge_params) < num_should_predict:
                    edge_params = np.pad(
                        edge_params,
                        (0, num_should_predict - len(edge_params)),
                        "constant",
                    )
                ...
                if edge_type == PanelEdgeTypeV3.MOVE:
                    ...
                    transl_params = transl_params * np.array(
                        self.gt_stats.translations.scale
                    ) + np.array(self.gt_stats.translations.shift)
                ...
                if self.encode_stitches_as_tags:
                    ...
                    if stitch_tag != -1:
                        ...
                        edge_stitches[stitch_tag].append(
                            {"panel": panel_name, "edge": num_edges}
                        )
                edge_dict = {}
                ...
                if edge_type == PanelEdgeTypeV3.CLOSURE_LINE:
                    ...
                    endpoint = edge_params * np.array(
                        self.gt_stats.vertices.scale
                    ) + np.array(self.gt_stats.vertices.shift)
                elif edge_type == PanelEdgeTypeV3.CURVE:
                    edge_params = self.get_bins_from_indices(edge_params)
                    edge_params = edge_params / self.bin_size
                    edge_params = (
                        edge_params.reshape(2, 2) * self.gt_stats.vertices.scale
                        + self.gt_stats.vertices.shift
                    )
                    abs_ctrl_pt, endpoint = edge_params[0], edge_params[1]
                    ctrl_pt = control_to_relative_coord(
                        last_point, endpoint, abs_ctrl_pt
                    )
                    edge_dict["curvature"] = {"type": "quadratic", "params": [ctrl_pt]}
                elif edge_type == PanelEdgeTypeV3.CLOSURE_CURVE:
                    edge_params = self.get_bins_from_indices(edge_params)
                    edge_params = edge_params / self.bin_size
                    edge_params = (
                        edge_params * self.gt_stats.vertices.scale
                        + self.gt_stats.vertices.shift
                    )
                    ctrl_pt = control_to_relative_coord(
                        last_point, np.array([0, 0]), edge_params
                    )
                    edge_dict["curvature"] = {"type": "quadratic", "params": [ctrl_pt]}
                    ...
                    edge_params = edge_params / self.bin_size
                    edge_params = (
                        edge_params.reshape(3, 2) * self.gt_stats.vertices.scale
                        + self.gt_stats.vertices.shift
                    )
                    abs_ctrl_pt1, abs_ctrl_pt2, endpoint = (
                        edge_params[0],
                        edge_params[1],
                        edge_params[2],
                    )
                    ctrl_pt1 = control_to_relative_coord(
                        last_point, endpoint, abs_ctrl_pt1
                    )
                    ctrl_pt2 = control_to_relative_coord(
                        last_point, endpoint, abs_ctrl_pt2
                    )
                    edge_dict["curvature"] = {
                        "type": "cubic",
                    ...
                    ctrl_pt1 = control_to_relative_coord(
                        last_point, np.array([0, 0]), edge_params[0]
                    )
                    ctrl_pt2 = control_to_relative_coord(
                        last_point, np.array([0, 0]), edge_params[1]
                    )
                    edge_dict["curvature"] = {
                        "type": "cubic",
                        "params": [ctrl_pt1, ctrl_pt2],
            ...
            transl_origin = panel_universal_transtation(
                np.array(panel_dict["vertices"]), panel_dict["rotation"], [0, 0, 0]
            )[1]
            shift = np.append(transl_origin, 0)  # to 3D
            panel_rotation = Rotation.from_euler(
                "xyz", panel_dict["rotation"], degrees=True
            )
            comenpensating_shift = -panel_rotation.as_matrix().dot(shift)
            panel_dict["translation"] = (
                np.array(panel_dict["translation"]) + comenpensating_shift
            )
        ...
        pattern["stitches"] = [
            stitches for stitches in edge_stitches.values() if len(stitches) == 2
        ]
