...
from .rotation import euler_xyz_to_R
from data.datasets.utils import PanelEdgeType, PanelEdgeTypeV2
...
class NNSewingPattern(VisPattern):
    ...
    def multi_pattern_as_tensors(patterns, pad_panels_to_len=None, pad_panels_num=None, pad_stitches_num=None,
            ...
            ret_panel_names=False, skip_none_panels=False, rotation_in_euler=False, ret_stitches_as_dict=False): 
        if sys.version_info[0] < 3:
        ...
        # Main info per panel
        panel_seqs, panel_translations, panel_rotations, aug_panel_seqs, panel_names = [], [], [], [], []
        for porder in panel_order:
            template_name, panel_name = porder
            if panel_name is not None and skip_none_panels:
                ...
                if len(cur_pat) == 0:
                    ...
                    edges, rot, transl, aug_edges = cur_pat[0].panel_as_numeric(panel_name, pad_to_len=max_len, rotation_in_euler=rotation_in_euler)
                    panel_name = cur_pat[0].panel_classifier.class_name(cur_pat[0].panel_classifier.class_idx(template_name, panel_name))
                elif len(cur_pat) > 1:
        ...
        if ret_stitches_as_dict:
            stitches_indices = []
            tags_per_edge = np.zeros((len(panel_seqs), len(panel_seqs[0]), 3))
            for pat in patterns:
                for idx, stitch in enumerate(pat.pattern['stitches']):
                    stitch_list = []
                    assert len(stitch) == 2
                    for side in stitch:
                        edge_id = side['edge']
                        panel_name = pat.panel_classifier.class_name(pat.panel_classifier.class_idx(pat.template_name, side['panel']))
                        stitch_list.append({'panel': panel_name, 'edge': edge_id})
                    stitches_indices.append(stitch_list)
        else:
    ...
    def pattern_vertices(self):
        """Return pattern vertices as a list of 3D points"""
        vertices = []
        for panel_name in self.panel_order():
            if panel_name is not None:
                vertices.append(self.panel_vertices(panel_name))
        return np.concatenate(vertices)
    
    def pattern_as_tokens(self, version=1, rot_in_euler=False, with_placement=False, with_stitches=False):
        ...
        """
        # get panel ordering
        panel_order = self.panel_order(pad_to_len=None)

        # Main info per panel
        panel_seqs, panel_translations, panel_rotations, panel_names = [], [], [], []
        for panel_name in panel_order:
            if panel_name is not None:
                edges, rot, transl = self.panel_as_list(panel_name, version=version, rot_in_euler=rot_in_euler)
                panel_seqs.append(edges)
                panel_translations.append(transl)
                panel_rotations.append(rot)
                p_name = self.panel_classifier.class_name(self.panel_classifier.class_idx(self.template_name, panel_name)) if self.panel_classifier is not None else panel_name
    ...
    def pattern_as_tensors(
        ...
        for panel_name in panel_order:
            if panel_name is not None:
                edges, rot, transl, aug_edges = self.panel_as_numeric(panel_name, pad_to_len=max_len, rotation_in_euler=rotation_in_euler)
            else:  # empty panel
    ...
    def pattern_from_tensors(
            ...
            padded=False, panel_names=None, silent=False, stitches_dict=None):
        ...
        self.pattern['stitches'] = stitches_dict if stitches_dict is not None else []
    ...
    def panel_as_numeric(self, panel_name, pad_to_len=None, rotation_in_euler=False):
        """
        ...
        rotation_representation = np.array(panel_rotation.as_quat()) if not rotation_in_euler else np.array(panel_rotation.as_euler('xyz', degrees=True))
        return np.stack(edge_sequence, axis=0), rotation_representation, translation, aug_edges
    
    
    
    def panel_vertices(self, panel_name):
        panel = self.pattern['panels'][panel_name]
        vertices = np.array(panel['vertices'])
        edge_sequence = [vertices[edge['endpoints']] for edge in panel["edges"]]
        curvatures = [np.array(edge['curvature']) if 'curvature' in edge else np.array([0, 0]) for edge in panel["edges"] ]
        abs_ctrl_pts = []
        for i, (edge, curvature) in enumerate(zip(edge_sequence, curvatures)):
            if not np.all(curvature == 0):
                abs_ctrl_pts.append(NNSewingPattern._control_to_abs_coord(edge[0], edge[1], curvature)[None])
        all_vertices = np.concatenate([vertices] + abs_ctrl_pts, 0)
        delta = edge_sequence[0][0].copy()
        return all_vertices - delta
    
    def panel_as_list(self, panel_name, version=1, rot_in_euler=False):
        """
            Represent panel as sequence of edges with each edge as vector of fixed length plus the info on panel placement.
            * Edges are returned in additive manner: 
                each edge as a vector that needs to be added to previous edges to get a 2D coordinate of end vertex
        ...
        if version == 1:
            panel_edge_type = PanelEdgeType
        elif version == 2:
            panel_edge_type = PanelEdgeTypeV2
        else:
            raise ValueError('BasicPattern::Error::panel_as_list() version {} is not supported'.format(version))
        ...
        for i, (edge, curvature) in enumerate(zip(edge_sequence, curvatures)):
            if np.all(curvature == 0):
                edge_type = panel_edge_type.LINE if i != num_edges - 1 else panel_edge_type.CLOSURE_LINE
                params = edge[1] - delta
            else:
                edge_type = panel_edge_type.CURVE if i != num_edges - 1 else panel_edge_type.CLOSURE_CURVE
                params = np.concatenate([NNSewingPattern._control_to_abs_coord(edge[0], edge[1], curvature), edge[1]]).reshape(2, 2) - delta
        ...
        panel_rotation = scipy_rot.from_euler('xyz', panel['rotation'], degrees=True)  # pattern rotation follows the Maya convention: intrinsic xyz Euler Angles
        rotation_representation = np.array(panel_rotation.as_quat()) if not rot_in_euler else panel_rotation.as_euler('xyz', degrees=True)
        return all_edges, rotation_representation, translation
    
    # def panel_vertices(self, panel_name):
    #     panel = self.pattern['panels'][panel_name]
    #     vertices = np.array(panel['vertices'])
    #     vertices_offset = vertices.min(0)
    #     return vertices - vertices_offset
    
    # def panel_as_list(self, panel_name):
    #     """
    #         Represent panel as sequence of edges with each edge as vector of fixed length plus the info on panel placement.
    #         * Edges are returned in additive manner: 
    #             each edge as a vector that needs to be added to previous edges to get a 2D coordinate of end vertex
    ...
    #         edge_token = []
    #         vector, curvature = edge[:2], edge[2:]
    #         if np.all(curvature == 0):
    #             edge_type = PanelEdgeType.LINE if i != num_edges - 1 else PanelEdgeType.CLOSURE_LINE
    #             params = vector
    #         else:
    #             edge_type = PanelEdgeType.CURVE if i != num_edges - 1 else PanelEdgeType.CLOSURE_CURVE
    #             params = edge
    #         edge_token.append(edge_type)
    #         edge_token.append(params)
    #         all_edges.append(edge_token)
    ...
    @staticmethod
    def _control_to_abs_coord_batch(start, end, control_scale):
        """
        Derives absolute coordinates of Bezier control point given as an offset
        """
        edge = end - start
        edge_perp = np.hstack([-edge[:, 1][..., np.newaxis], edge[:, 0][..., np.newaxis]])

        control_start = start + control_scale[:, 0][..., np.newaxis] * edge
        control_point = control_start + control_scale[:, 1][..., np.newaxis] * edge_perp

        # get the center point on the curve
        center_point = (1 - 0.7) ** 2 * start + 2 * 0.7 * (1 - 0.7) * control_point + 0.7 ** 2 * end

        return control_point, center_point 
    ...
    def edge_augments_batch(self, edge_sequence):
        ...
        center_points = NNSewingPattern._control_to_abs_coord_batch(starts, ends, control_scale)[1]
        ...
        aug_edges = all_points[end_idx.astype(np.int64)] - all_points[start_idx.astype(np.int64)]

        return all_points, aug_edges

    def edge_augments(self, edge_sequence, i):
        aug_points = [np.array([0, 0])]
        ori_points = [np.array([0, 0])]
        for idx in range(len(edge_sequence)):
            start_point = aug_points[-1]
            edge_info = edge_sequence[idx]
            next_vert = start_point + edge_info[:2]
            center_point = NNSewingPattern._control_to_abs_coord_batch(start_point.reshape(1, -1), 
                                                                 next_vert.reshape(1, -1), 
                                                                 edge_info[2:].reshape(1, -1))[1]
            aug_points.append(center_point[0])
            aug_points.append(next_vert)
            ori_points.append(next_vert)

        NNSewingPattern.vis_aug_points(edge_sequence, ori_points, i + str(1))
        NNSewingPattern.vis_aug_points(edge_sequence, aug_points, i + str(100))

        
        NNSewingPattern.vis_aug_points(edge_sequence, aug_points, i)
        aug_edges = []
        for i in range(len(aug_points) - 1):
            for j in range(i + 1, len(aug_points)):
                aug_edges.append(aug_points[j] - aug_points[i])

        return np.stack(aug_points), np.stack(aug_edges)    

    @staticmethod
    def vis_aug_points(edge_sequence, aug_points, x=0):
        ...
        fig = plt.figure(figsize=(5.75, 4), dpi=96)
        plt.axis('off')
        for idx in range(edge_sequence.shape[0]):
            edge_info = edge_sequence[idx]
            next_vert = start_point + edge_info[:2]
            
            control_point = NNSewingPattern._control_to_abs_coord_batch(start_point.reshape(1, -1), 
                                                                  next_vert.reshape(1, -1), 
                                                                  edge_info[2:].reshape(1, -1))[0][0]
            X = (start_point, control_point, next_vert)
            xx = P(np.linspace(0, 1, 200), X)
            plt.plot(xx[:,0], xx[:,1], 'k-') 
            start_point = next_vert
        plt.savefig(f"paper_vis_panel_only_{x}.png")
        aug_points = np.stack(aug_points)

        plt.plot(aug_points[:, 0], aug_points[:, 1], "ro")
        plt.savefig(f"paper_vis_panel_with_points_{x}.png")
        for i in range(aug_points.shape[0] - 1):
            for j in range(i, aug_points.shape[0]):
                plt.arrow(aug_points[j, 0], 
                          aug_points[j, 1], 
                          aug_points[i, 0] - aug_points[j, 0], 
                          aug_points[i, 1] - aug_points[j, 1], 
                          head_width=1, head_length=1, fc='b', ec='b')
                if i == 0 and j == i+1:
                    plt.axis
                    plt.savefig(f"paper_vis_panel_with__01edge_{x}.png", facecolor="none")
            if i == 0:
                plt.savefig(f"paper_vis_panel_with_1edge_{x}.png", facecolor="none")
        plt.savefig(f"paper_vis_panel_all_{x}.png", facecolor="none")
        plt.close(fig)
    ...
    def _empty_panel(self, max_edge_num):
        ...
        return np.zeros((max_edge_num, 4)), np.zeros(4), np.zeros(3), np.zeros((max_edge_num * (2 * max_edge_num + 1), 2))
