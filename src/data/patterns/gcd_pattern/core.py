...
from .utils import arc_from_three_points, arc_rad_flags_to_three_point
...
class BasicPattern(object):
    ...
    # --------- Info ------------------------
    def panel_order(self, force_update=False, **kwargs):
        """
    ...
    def _edge_as_vector(self, vertices, edge_dict):
        ...
        if 'curvature' in edge_dict:
            ...
            # in case of an arc
                ...
                curvature = np.array(local_coords + local_coords + [1])

            # in case of a curve
            elif edge_dict['curvature']['type'] == 'cubic':
                curvature = np.array(edge_dict['curvature']['params'][0] +
                                     edge_dict['curvature']['params'][1] + [0])
                
            elif edge_dict['curvature']['type'] == 'quadratic':
                # Convert to cubic for uniformity
                # https://stackoverflow.com/questions/3162645/convert-a-quadratic-bezier-to-a-cubic-one
                # NOTE: Assuming relative coor
                cp = np.array(edge_dict['curvature']['params'][0])
                start, end = np.array([0, 0]), np.array([1, 0])
