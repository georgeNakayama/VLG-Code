...
class VisPattern(core.ParametrizedPattern):
    ...
    def serialize(
        ...
        # svg_clean_file = os.path.join(log_dir, (self.name + tag + '_pattern_clean.svg'))
        png_file = os.path.join(log_dir, (self.name + tag + '_pattern.png'))
        # png_clean_file = os.path.join(log_dir, (self.name + tag + '_pattern_clean.png'))
    ...
    def _save_as_image(
        ...
        if len(paths) == 0:
            raise RuntimeError(f'{self.__class__.__name__}::{self.name}::ERROR::Cannot evaluate BBox: no panels specified') 
