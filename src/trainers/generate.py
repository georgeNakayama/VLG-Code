...
def generate(
    ...
    for i, input_dict in enumerate(loader):
        ...
        output_text, patterns, error_type = garment_tokenizer.decode_tensor(
        ...
        try:
            ...
            patterns.serialize(
                os.path.join(log_dir, data_name),
                spec_only=False,
                with_3d=False,
                with_text=False,
                view_ids=False,
        ...
        modewise_edge_accs = {}
        total_edge_acc = num_edge_accs[num_edge_accs != -1].sum() / max(
            (num_edge_accs != -1).sum(), 1
        )
        modewise_edge_correct_accs = {}
        total_edge_correct_acc = num_edge_correct_accs[
            num_edge_correct_accs != -1
        ].sum() / max((num_edge_correct_accs != -1).sum(), 1)
        modewise_vertex_L2s = {}
        total_vertex_L2 = vertex_L2s[vertex_L2s != -1].sum() / max(
            (vertex_L2s != -1).sum(), 1
        )
        modewise_transl_l2s = {}
        total_transl_l2 = transl_l2s[transl_l2s != -1].sum() / max(
            (transl_l2s != -1).sum(), 1
        )
        modewise_rots_l2s = {}
        total_rots_l2 = rots_l2s[rots_l2s != -1].sum() / max((rots_l2s != -1).sum(), 1)
        modewise_stitch_accs = {}
        total_stitch_acc = stitch_accs[stitch_accs != -1].sum() / max(
            (stitch_accs != -1).sum(), 1
        )
        ...
        if ddp_rank == 0:
            ...
            for mode in modewise_panel_accs.keys():
                log_dict.update(
                    {
                        f"val/{mode}_num_panel_accuracy": modewise_panel_accs[mode]
                        .cpu()
                        .item(),
                        f"val/{mode}_num_edge_accuracy": modewise_edge_accs[mode]
                        .cpu()
                        .item(),
                        f"val/{mode}_num_correct_edge_accuracy": modewise_edge_correct_accs[
                            mode
                        ]
                        .cpu()
                        .item(),
                        f"val/{mode}_vertex_L2": modewise_vertex_L2s[mode].cpu().item(),
                        f"val/{mode}_translation_L2": modewise_transl_l2s[mode]
                        .cpu()
                        .item(),
                        f"val/{mode}_rotation_L2": modewise_rots_l2s[mode].cpu().item(),
                        f"val/{mode}_stitch_acc": modewise_stitch_accs[mode]
                        .cpu()
                        .item(),
                    }
                )
