...
class MainConfig:
    ...
    optimizer: dict = field(
        default_factory=lambda: {"lr": 1e-4, "beta1": 0.9, "beta2": 0.999}
    )
...
def add_tokens_to_garment_tokenizer(
    garment_tokenizer: GarmentTokenizer, processor
) -> tuple[int, dict]:
    processor.tokenizer.pad_token = "<|finetune_right_pad_id|>"
    all_new_tokens = garment_tokenizer.get_all_token_names()
    num_added_tokens = processor.tokenizer.add_tokens(
        all_new_tokens, special_tokens=True
    )
...
def get_model(
    cfg: MainConfig, ddp_local_rank
) -> AIpparelMllavaNextForConditionalGeneration:
...
def add_tokens_to_model_config(
    model: AIpparelMllavaNextForConditionalGeneration,
    ...
    model.config.quadratic_token_index = (
        garment_tokenizer.panel_edge_type_indices.curve_idx
    )
    ...
    model.config.cline_token_index = (
        garment_tokenizer.panel_edge_type_indices.closure_line_idx
    )

    model.config.cquadratic_token = PanelEdgeTypeV3.CLOSURE_CURVE.value
    model.config.cquadratic_token_index = (
        garment_tokenizer.panel_edge_type_indices.closure_curve_idx
    )

    model.config.ccubic_token = PanelEdgeTypeV3.CLOSURE_CUBIC.value
    model.config.ccubic_token_index = (
        garment_tokenizer.panel_edge_type_indices.closure_cubic_idx
    )

    model.config.carc_token = PanelEdgeTypeV3.CLOSURE_ARC.value
    model.config.carc_token_index = (
        garment_tokenizer.panel_edge_type_indices.closure_arc_idx
    )
...
def main(cfg: MainConfig):
    ...
    dataset_train = hydra.utils.instantiate(cfg.dataset, split="train")
    dataset_val = hydra.utils.instantiate(cfg.dataset, split="val")
    if master_process:
    ...
    processor.tokenizer.pad_token = "<|finetune_right_pad_id|>"
    num_added_tokens, token_name2_idx_dict = add_tokens_to_garment_tokenizer(
        garment_tokenizer, processor
    )
    ...
    if master_process:
        log.info(
            f"Total parameters: {total_params}, Trainable parameters: {trainable_params} ({trainable_params/total_params:.4f})"
        )
    ...
    if not (cfg.eval_only or cfg.gen_only):
        ...
        assert not (
            cfg.eval_only and cfg.gen_only
        ), "Only eval or gen can be set at a time!"
        ...
        batch_size = 1 if cfg.gen_only else cfg.batch_size
        ...
        if cfg.eval_only:
            ...
            generation_output_dir = os.path.join(
                output_dir, f"generation_outputs_{cfg.gen_split}"
            )
